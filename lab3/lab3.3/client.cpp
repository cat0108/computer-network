...
int Window::add_to_window(char* msg,int msg_size,uint8_t seq)
{
    ...
    this->timer_list.push_back(temp);
    temp->start_timer(seq);
    next++;
    return 0;
}
...
int Window::update_window_br(uint8_t ack)
{
    //判断是否为空
    if(next==0)
    {
    ...
    for(i=0;i<next;i++)
    {
        if(update_status[i]==true)
        {
            ...
            this->timer_list.erase(this->timer_list.begin());
        }
        else
        {
            break;
